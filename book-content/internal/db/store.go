...
type Book struct {
	...
	// Genres is comma-separated (e.g. "Fiction, Gothic, Horror"), 3-5
	// tags, straight from the catalog — see pipeline/internal/catalog.
	// Entry.Genres's own doc comment for why a raw TEXT value rather
	// than a Postgres array; split on "," client-side wherever a list
	// is actually needed for display.
	Genres string `json:"genres"`
	// Summary is a short synopsis, hand-curated per book same as
...
func (s *Store) ListBooks(ctx context.Context) ([]Book, error) {
	const q = `
		SELECT id, gutenberg_id, title, author, source_url, level, target_language, native_language, status, genres, summary
		FROM book
	...
	for rows.Next() {
		var b Book
		if err := rows.Scan(&b.ID, &b.GutenbergID, &b.Title, &b.Author, &b.SourceURL, &b.Level, &b.TargetLanguage, &b.NativeLanguage, &b.Status, &b.Genres, &b.Summary); err != nil {
...
func (s *Store) GetBook(ctx context.Context, bookID int) (Book, error) {
	...
	var b Book
	err := s.db.QueryRow(ctx, q, bookID).Scan(&b.ID, &b.GutenbergID, &b.Title, &b.Author, &b.SourceURL, &b.Level, &b.TargetLanguage, &b.NativeLanguage, &b.Status, &b.Genres, &b.Summary)
