...
- ~~**i18n architecture**~~ — resolved: the pipeline takes an explicit language pair per run (no default), not a hardcoded English/Japanese assumption. See §7i.
...
**Catalog file** (`pipeline/catalogs/<pair>.txt` — one file per language pair, see §7i — parsed by `pipeline/internal/catalog`): a plain text file, one entry per book. Each entry is exactly 6 lines — title, author, Gutenberg URL (the plain-text edition specifically; `Clean` depends on Gutenberg's own START/END markers, which only that edition has), first line of the actual novel content, last line of the actual novel content, `Level=X` — separated from other entries by a blank line; a `# comment` line is ignored by the parser wherever it appears (a section header, or disabling an entry — no longer needed per-entry now that title/author are real fields). See a catalog file's own header comment, or the README's "Adding a Book to the Catalog" section, for the full spec.
...
**Orchestration** (`pipeline.PrepareBook`, in `pipeline/internal/pipeline`): `Fetch → Clean → Trim` for one `catalog.Entry`, returning the final novel-only text. `cmd/ingest` is the runnable entrypoint — reads a catalog file (`-catalog`, defaulting to `catalogs/EN_JP.txt`; this command doesn't need to know about language pairs at all, since it never touches question/breakdown generation), calls `PrepareBook` for every entry, then also runs Stage A (`segment.Segment`) on the result and writes two output files per book to `pipeline/books/` (gitignored — generated, not source; unrelated to `pipeline/catalogs/` despite the similar name): `{id}.txt` (the prepared novel text) and `{id}.sentences.txt` (its sentences, one per line, indexed with char counts, for human inspection — not a format anything downstream parses; Stage B takes `[]types.SentenceInput` directly, in memory). Continues past a single book's failure rather than aborting the whole run, but exits non-zero if anything failed. Stage B (LLM chunk grouping) and beyond are not run by this command.
...
## 7i. Implementation Handoff — Language Pair / i18n Architecture (Go)

Implemented in `pipeline/internal/langpair`. A `LanguagePair` bundles `Target` (ISO 639-1 code of the language the learner is studying — what the source text and question prompts about it are written in) and `Native` (ISO 639-1 code of the learner's own language — what questions/options/explanations and the breakdown are written in), plus breakdown's per-pair formatting (`BreakdownSectionLabels`, an optional `BreakdownExample`) and an optional `ValidateNativeText` script-check. `DisplayName(code)` turns a code into the human-readable English name (`"en"` → `"English"`) that prompts actually interpolate into their instructional prose — codes are what's stored/compared everywhere (`LanguagePair.Target`/`Native`, `book.target_language`/`native_language`), display names exist only for building natural-sounding prompt text. Two pairs exist today: `EN_JP` (English source, Japanese explanations — v1's shipped pair) and `JP_EN` (the reverse). `JP_EN`'s validator deliberately checks the *absence* of Japanese script rather than the presence of English — there's no cheap way to positively confirm "this is English" by inspecting text alone, but checking the model didn't just ignore instructions and answer in the target language anyway still catches the real failure mode.

**No default pair, anywhere.** `question.Generator`/`breakdown.Generator` both return an error immediately if `LanguagePair` is left unset, rather than silently assuming one — a pair is always an explicit choice. `cmd/process`'s `-pair=EN_JP|JP_EN` flag is required; it resolves both the `LanguagePair` passed to the generators and which catalog file supplies the books (`pipeline/catalogs/<pair>.txt`, one file per pair, same 6-line-per-entry format as before — the pair isn't a per-entry field, since a whole catalog run is one pair). `cmd/livetest` hardcodes `EN_JP` (it's a throwaway smoke test, not meant to be pair-flexible); `cmd/ingest` doesn't need a pair at all — it only cleans/segments source text, never touches question/breakdown generation.

**Database**: one pair is a property of the *book*, not something duplicated onto `chunk`/`question`/`breakdown`/`user_progress` — those already scope to a pair transitively via `book_id`. `book.language` (previously unused beyond defaulting to `"en"`) became two required columns, `target_language` and `native_language`, named to match `LanguagePair`'s own fields.

Deliberately **not** a per-book field for *choosing* a pair (that stays a whole-catalog-run decision via `-pair`) — the DB columns just record, after the fact, which pair a given book was processed under.
